...
export async function getListPermissionByRoleAction(roleId: string): Promise<TResAction<TResPagination<TListPermissionByRole> | null>> {
   try {
      const result = await api.get<TRes<TResPagination<TListPermissionByRole>>>(`${ENDPOINT.PERMISSION.LIST_PERMISSION_BY_ROLE}/${roleId}?pageSize=100`);
      const { data } = result;
...
export async function getPermissionsAction(query: string): Promise<TResAction<TResPagination<TPermission> | null>> {
    try {
        const result = await api.get<TRes<TResPagination<TPermission>>>(`${ENDPOINT.PERMISSION.PERMISSION}?${query}`);
        const { data } = result;
        return { status: "success", message: result.message, data: data };
    } catch (error: any) {
        return { status: "error", message: error?.message, data: null };
...
export async function createPermissionsAction(payload: TCreatePermissionAction): Promise<TResAction<boolean | null>> {
    try {
        const result = await api.post<TRes<boolean>>(`${ENDPOINT.PERMISSION.PERMISSION}`, payload);
        const { data } = result;
        return { status: "success", message: result.message, data: data };
    } catch (error: any) {
        return { status: "error", message: error?.message, data: null };
...
export const updatePermissionsAction = async (payload: TUpdatePermissionAction): Promise<TResAction<TPermission | null>> => {
    try {
        const result = await api.patch<TRes<TPermission>>(`${ENDPOINT.PERMISSION.PERMISSION}/${payload._id}`, payload);
        const { data } = result;
        return { status: "success", message: result.message, data: data };
    } catch (error: any) {
        return { status: "error", message: error?.message, data: null };
...
export async function deletePermissionsAction(payload: string): Promise<TResAction<boolean | null>> {
    try {
        const result = await api.delete<TRes<any>>(`${ENDPOINT.PERMISSION.PERMISSION}/${payload}`);
        const { data } = result;
        return { status: "success", message: result.message, data: data };
    } catch (error: any) {
        return { status: "error", message: error?.message, data: null };
