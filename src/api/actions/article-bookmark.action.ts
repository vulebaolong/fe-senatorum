...
import { ArticleBookmark, TArticleBookmarkReq } from "@/types/article-bookmark.type";
...
export async function getAllArticleBookmarkAction() {
    try {
        const result = await api.get<TRes<ArticleBookmark[]>>(`${ENDPOINT.ARTICLE_BOOKMARK.ARTICLE_BOOKMARK_ALL}`);
        const { data } = result;
        return { status: "success", message: result.message, data: data };
    } catch (error: any) {
        return { status: "error", message: error?.message, data: null };
